...
let graphScale = 1, graphPanX = 0, graphPanY = 0; let graphTime = 0; function buildGraph() {
    graphScale = 1;
    ...
    const idMap = {}, childrenOf = {}, globalRoots = []; let maxLevel = 0; savedList.forEach(r => {
        if (r.isFolderMeta) return;
        const parts = (r.folder || 'Uncategorized').split('/'); let parentId = null; parts.forEach((p, i) => {
            const fid = 'folder_' + parts.slice(0, i + 1).join('/');
            if (!idMap[fid]) {
                const lvl = i; if (lvl > maxLevel) maxLevel = lvl; const fn = { id: fid, label: p, type: 'folder', level: lvl, x: cx, y: cy, vx: 0, vy: 0, fixed: !!globalLayout[fid], phase: Math.random() * Math.PI * 2 };
                if (globalLayout[fid]) { fn.x = globalLayout[fid].x; fn.y = globalLayout[fid].y } graphNodes.push(fn);
                idMap[fid] = fn; if (parentId) {
                    graphLinks.push({ source: idMap[parentId], target: fn, len: 150 });
                    if (!childrenOf[parentId]) childrenOf[parentId] = []; if (!childrenOf[parentId].includes(fid)) childrenOf[parentId].push(fid)
                }
                else { globalRoots.push(fid) } childrenOf[fid] = []
            } parentId = fid
        ...
        if (parentId) {
            graphLinks.push({ source: idMap[parentId], target: rn, len: 120 }); if (!childrenOf[parentId]) childrenOf[parentId] = [];
            if (!childrenOf[parentId].includes(r.id)) childrenOf[parentId].push(r.id)
        } else { globalRoots.push(r.id) }
        idMap[r.id] = rn
    }); const ringRadius = 150; function layoutRadial(nodeId, angle, arc, depth) {
        const children = childrenOf[nodeId] || [];
        if (children.length === 0) return; const parent = idMap[nodeId]; const childArc = arc / Math.max(1, children.length);
        let startAngle = angle - arc / 2 + childArc / 2; children.forEach((cid, i) => {
    ...
    if (GR_count <= 2) clusterSeparation = 250; globalRoots.forEach((grid, i) => {
        if (globalLayout[grid]) return;
    ...
    graphNodes.forEach(n => {
        n.vx = 0; n.vy = 0; const el = document.createElement('div'); el.className = 'gn-node ' + (n.type === 'folder' ? 'folder' : '');
        ...
        el.onmousedown = (e) => {
            ...
            const oy = (n.y * graphScale + graphPanY) - e.clientY; const mm = (me) => {
                n.x = (me.clientX + ox - graphPanX) / graphScale;
                n.y = (me.clientY + oy - graphPanY) / graphScale
            }; const mu = () => {
                draggedNode = null; storeSaved(true);
                document.removeEventListener('mousemove', mm); document.removeEventListener('mouseup', mu)
            };
            document.addEventListener('mousemove', mm); document.addEventListener('mouseup', mu)
        };
        el.ondblclick = (e) => {
            e.preventDefault(); e.stopPropagation(); if (n.type === 'request') {
                closeGraph();
                createTab(savedList.find(x => x.id === n.id))
            }
        }; D_graphNodes.appendChild(el)
    }); D_graphLayer.onmousedown = (e) => {
        if (e.target !== D_graphLayer && e.target !== D_graphCanvas && e.target !== D_graphNodes) return;
        if (e.button !== 0 && e.button !== 2) return; e.preventDefault(); const sx = e.clientX, sy = e.clientY;
        const startPanX = graphPanX, startPanY = graphPanY; const mm = (me) => {
            graphPanX = startPanX + (me.clientX - sx);
            graphPanY = startPanY + (me.clientY - sy)
        }; const mu = () => {
            document.removeEventListener('mousemove', mm);
            document.removeEventListener('mouseup', mu)
        }; document.addEventListener('mousemove', mm);
        document.addEventListener('mouseup', mu)
    }; D_graphLayer.onwheel = (e) => {
        e.preventDefault();
...
function physicsLoop() {
    if (!graphActive) return; graphTime += 0.015; const ctx = D_graphCanvas.getContext('2d');
    const W = D_graphLayer.offsetWidth, H = D_graphLayer.offsetHeight; if (D_graphCanvas.width !== W || D_graphCanvas.height !== H) {
        D_graphCanvas.width = W;
        D_graphCanvas.height = H
    } const centerX = W / 2, centerY = H / 2; const N = graphNodes.length; const minDist = 90;
    for (let i = 0; i < N; i++) {
        const a = graphNodes[i]; for (let j = i + 1; j < N; j++) {
            const b = graphNodes[j];
            if (a.fixed && b.fixed) continue; let dx = a.x - b.x, dy = a.y - b.y; let dist2 = dx * dx + dy * dy; if (dist2 < 1) {
                dx = Math.random() - 0.5;
                dy = Math.random() - 0.5; dist2 = 1
            } const dist = Math.sqrt(dist2); let f = Math.min(1200 / dist2, 4);
            if (dist < minDist) f += (minDist - dist) * 0.3; const fx = (dx / dist) * f, fy = (dy / dist) * f; if (!a.fixed) {
                a.vx += fx;
                a.vy += fy
        ...
        const fx = (dx / dist) * spring, fy = (dy / dist) * spring; if (!l.source.fixed) {
            l.source.vx += fx;
            l.source.vy += fy
        } if (!l.target.fixed) { l.target.vx -= fx; l.target.vy -= fy }
    } const damping = 0.9;
    for (let i = 0; i < N; i++) {
        const n = graphNodes[i]; if (n.fixed) { n.vx = 0; n.vy = 0 } else {
    ...
    ctx.beginPath(); for (let i = 0; i < graphLinks.length; i++) {
        const l = graphLinks[i]; ctx.moveTo(l.source.x + l.source.wx, l.source.y + l.source.wy);
        ctx.lineTo(l.target.x + l.target.wx, l.target.y + l.target.wy)
    } ctx.strokeStyle = 'rgba(200,97,47,0.30)';
    ctx.lineWidth = 1.5; ctx.stroke(); graphNodes.forEach(n => { n.el.classList.remove('gn-child', 'gn-descendant') });
    const target = hoveredNode || draggedNode; if (target) {
        let queue = [target]; let visited = new Set([target.id]);
        let isFirstLevel = true; while (queue.length > 0) {
            let nextQueue = []; for (let parent of queue) {
                for (let l of graphLinks) {
                    if (l.source === parent && !visited.has(l.target.id)) {
                        visited.add(l.target.id);
                        if (isFirstLevel) l.target.el.classList.add('gn-child'); else l.target.el.classList.add('gn-descendant');
                        nextQueue.push(l.target)
                    }
                }
            } queue = nextQueue; isFirstLevel = false
        }
    } animFrame = requestAnimationFrame(physicsLoop)
}
D_mapViewBtn.onclick = () => {
    const active = D_graphLayer.style.display === 'block'; if (active) { closeGraph() }
    else {
        graphActive = true; D_graphLayer.style.display = 'block'; D_graphNodes.style.display = 'block';
        D_graphCanvas.style.display = 'block'; if (animFrame) cancelAnimationFrame(animFrame);
        if (graphNeedsRebuild) { buildGraph(); graphNeedsRebuild = false } physicsLoop()
    }
}; D_closeGraphBtn.onclick = closeGraph;
